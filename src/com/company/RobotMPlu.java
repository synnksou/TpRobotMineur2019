...
public class RobotMPlu extends Robot {

    public RobotMPlu(int id,int x,int y){
        super(id,x,y);
    }
    ...
    public void miner(Mine mine) {
        MinePlu mineUra = (MinePlu) mine;
        int temporyStock =mineUra.getStock();
        if(!mineUra.stockIsNull()) {
            mine.setStock(--temporyStock);
            setStock(1 + this.getStock());}
        else {
